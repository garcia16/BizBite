...
import 'addReportStorage_screen.dart';
...
class _StorageReportViewState extends State<StorageReportView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarToBack(context, 'Informes de Almacén'),
      body: Column(
        children: [
          _buildFilters(),
          Expanded(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context, 
            MaterialPageRoute(builder: (context) => AddStorageReportView())
          ).then((_) async {
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (report.includesDetail) const Row(
              children: [
                Icon(Icons.arrow_right),
                Text('Incluye histórico de reposición de stock'),
              ],
            ),
          ],
        ),
  ...
  void _filterReports() {
    setState(() {
      filteredReports = reportsList.where((report) {
        ...
        // Comprobamos si el tipo de informe es 'Storage'
        bool typeMatches = report.typeReport == 'Storage';
